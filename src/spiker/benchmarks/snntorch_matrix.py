"""Compare compiled spiker examples against eager snnTorch across timesteps."""
...
from spiker.benchmarks.mnist_compare import Result, parse_variants, run_variant
...
def print_markdown(args: argparse.Namespace, rows: list[MatrixRow]) -> None:
    print("# spiker vs snnTorch Matrix")
    print()
    print("Compares compiled spiker examples against eager snnTorch examples across timesteps.")
    print()
    print("## Environment")
    print()
    ...
    for timesteps, results in by_timestep.items():
        ...
        for label, spiker_result, snntorch_result in comparisons:
            if spiker_result is None or snntorch_result is None:
                continue
            step_speedup = speedup(
                snntorch_result.steady_state_step_ms,
                spiker_result.steady_state_step_ms,
            )
            memory_ratio = speedup(
                snntorch_result.peak_cuda_memory_mb,
                spiker_result.peak_cuda_memory_mb,
            )
...
def main() -> None:
    ...
    parser.add_argument("--conv-synapse-init", choices=("spiker", "fan_in"), default="fan_in")
